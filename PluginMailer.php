...
class PluginMailer extends ServicePlugin
{
    ...
    function execute()
    {
        ...
        $messages = array();
        $numMailsSent = 0;
        $query = "SELECT id, subject, `from`, from_name, bcc, priority, confirmreceipt, emailtype, body, contenttype, dfilename, attachment FROM email_queue";
        ...
        while ($row = $result->fetch()) {
            ...
            while ($row2 = $result2->fetch()) {
                ...
                try {
                    $mailSend = $mailGateway->sendMailMessage(  $body,
                                                                $row['from'],
                                                                $row['from_name'],
                                                                $row2['userid'],
                                                                $row['bcc'],
            ...
            if ($failedAddressees) {
                ...
                CE_Lib::log(1, "Error trying to E-mail queued message to user(s) $users");
                $messages[] = new CE_Error($this->user->lang('Error trying to E-mail queued message to user(s) %s', $users));
            } else {
	...
	function pendingItems()
	{
		...
		$query = "SELECT id, subject, bcc, body, emailtype, attachment FROM email_queue";
        $result = $this->db->query($query);
        $queueEmpty = true;
        $mailGateway = new NE_MailGateway();
        $limit = 100;
